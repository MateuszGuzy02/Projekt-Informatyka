<?php
    ...
    if(isset($_GET['del'])) {
        $pol = mysqli_connect("localhost","mateusz","mateusz","text");
?>
    <form class="form" method="POST" action="?opcja=stronaglowna&del=<?php echo $_GET['del']; ?>">
        Czy na pewno usunąć? 
        <input type="submit" class="wyslij" value="TAK" name="tak" />
        <input type="submit" class="wyslij" value="NIE" name="nie" />
    </form>
<?php
    }
